...
#define CONFIG_GROUP_APP_DETECTOR_OUTPUT "generate-detector-output"
#define CONFIG_GROUP_APP_TRACKER_OUTPUT "generate-tracker-output"
...
parse_app (NvDsConfig * config, GKeyFile * key_file, gchar * cfg_file_path)
{
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0 (*key, CONFIG_GROUP_APP_ENABLE_PERF_MEASUREMENT)) {
      ...
      CHECK_ERROR (error);
    } else if (!g_strcmp0 (*key, CONFIG_GROUP_APP_DETECTOR_OUTPUT)) {
      config->detector_output = g_key_file_get_integer (key_file, CONFIG_GROUP_APP,
        CONFIG_GROUP_APP_DETECTOR_OUTPUT, &error);
      CHECK_ERROR (error);
    } else if (!g_strcmp0 (*key, CONFIG_GROUP_APP_TRACKER_OUTPUT)) {
      config->tracker_output = g_key_file_get_integer (key_file, CONFIG_GROUP_APP,
        CONFIG_GROUP_APP_TRACKER_OUTPUT, &error);
      CHECK_ERROR (error);
